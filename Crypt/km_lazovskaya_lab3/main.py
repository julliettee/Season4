...
def without_last_block(arr, type_):
    if type_ == 0 or type_ == 1 or type_ == 2:
        cnt = arr[len(arr) - 1]
        while cnt:
            arr.pop()
            cnt -= 1
...
def make_msg_array(str_msg):
    j = 0
    for i in range(len(str_msg)):
        if j == 0:
            msg.append(0)
        n = len(msg) - 1
        msg[n] |= (ord(str_msg[i]) << (j * 8))
        j += 1
        j %= 4


def convert_to_str(arr_msg):
    res = ""
    for ch in arr_msg:
        byte = ch
        for i in range(0, 4):
...
class MyBlock:
    def __init__(self, A, B, C, D, key_start):
        self.sub = []
        self.sub.append(A)
        self.sub.append(B)
        self.sub.append(C)
        self.sub.append(D)
        self.K = MyKey(key_start)

    def ForwardPass(self, i):
        # обращаемся к 4-ем S-блокам
        self.sub[1] ^= MARSInfo.S0[GetByte(self.sub[0], 0)]
        self.sub[1] += MARSInfo.S1[GetByte(self.sub[0], 1)]
        self.sub[2] += MARSInfo.S0[GetByte(self.sub[0], 2)]
        self.sub[3] ^= MARSInfo.S1[GetByte(self.sub[0], 3)]

        RightCyclicShift(self.sub[0], 24)
        # также проделаем дополнительные операции смешивания
        if i == 0 or i == 4:
            self.sub[0] += self.sub[3]
        if i == 1 or i == 5:
            self.sub[0] += self.sub[1]

        self.sub.append(self.sub[0])
        self.sub.pop(0)

    def DForwardPass(self, i):
        self.sub.insert(0, self.sub[len(self.sub) - 1])
        self.sub.pop()

        # и вращаем исходное слово вправо]
        RightCyclicShift(self.sub[0], 24)
        # обращаемся к 4-ем элементам S-блоков
        self.sub[3] ^= MARSInfo.S0[GetByte(self.sub[0], 1)]
        self.sub[3] += MARSInfo.S1[GetByte(self.sub[0], 2)]
        self.sub[2] += MARSInfo.S0[GetByte(self.sub[0], 3)]
        self.sub[1] ^= MARSInfo.S1[GetByte(self.sub[0], 0)]
    ...
    def BackwardPass(self, i):
        ...
        LeftCyclicShift(self.sub[0], 24)
        a = (MARSInfo.S1[GetByte(self.sub[0], 0)])
        self.sub.append(self.sub[0])
        self.sub.pop(0)

    def DBackwardPass(self, i):
        # вращаем массив D[]
        self.sub.insert(0, self.sub[len(self.sub) - 1])
        self.sub.pop()
        ...
        # и вращаем исходное слово влево
        LeftCyclicShift(self.sub[0], 24)
        # обращаемся к 4-ем элементам S-блоков
        self.sub[3] ^= MARSInfo.S1[GetByte(self.sub[0], 3)]
        self.sub[2] -= MARSInfo.S0[GetByte(self.sub[0], 2)]
    ...
    def E(self, sub, i):
        M = sub
        M += self.K.getElement(2 * i + 4)
        R = sub
        LeftCyclicShift(R, 13)
        R *= self.K.getElement(2 * i + 5)
        i = GetBites(M, 9)
        L = 0
        if i < 256:
            L = MARSInfo.S0[i]
            ...
            L = MARSInfo.S1[i % 256]
        LeftCyclicShift(M, GetBites(R, 5))
        L ^= R
        LeftCyclicShift(R, 5)
        L ^= R
        LeftCyclicShift(L, GetBites(R, 5))
        return [L, M, R]

    def Crypto(self, i):
        O = self.E(self.sub[0], i)
        LeftCyclicShift(self.sub[0], 13)
        self.sub[2] += O[1]
    ...
    def DCrypto(block, i):
        # вращаем массив sub[]
        block.sub.insert(0, block.sub[len(block.sub) - 1])
        block.sub.pop()

        # и вращаем исходное слово вправо
        RightCyclicShift(block.sub[0], 13)
        O = block.E(block.sub[0], i)
        block.sub[2] -= O[1]
    ...
    def AddKey(self):
        for i in range(0, 4):
            self.sub[i] += self.K.getElement(i)

    def DAddKey(self):
        for i in range(0, 4):
            self.sub[i] += self.K.getElement(36 + i)
...
class Mars:
    ...
    def encrypt(self, type_):
        make_last_block(msg, type_)
        print("msg after adding last block = ", msg)
        blocks = []
        i = 0
        print("key = ", self.key.K)
        while i < len(msg):
            block = MyBlock(msg[i], msg[i + 1], msg[i + 2], msg[i + 3], self.key)
            # наложение ключа на блок
            block.AddKey()
            # 8 раундов прямого перемешивания
            for j in range(0, 8):
                block.ForwardPass(j)
            # 8 раундов обратного и 8 раундов прямого криптопреобразования
            for j in range(0, 16):
                block.Crypto(j)
            # 8 раундов прямого перемешивания
            for j in range(0, 8):
                block.BackwardPass(j)
            # наложение фрагментов расширенного ключа
            block.SubKey()
            blocks.append(block)
            i += 4
        return blocks
    ...
    def decrypt(self, blocks):
        ...
        for i in range(len(blocks)):
            ...
            while j >= 0:
                ...
                j -= 1
            j = 15
            # 8 раундов обратного и 8 раундов обратного криптопреобразования
            while j >= 0:
                blocks[i].DCrypto(j)
                j -= 1
            j = 7
...
if __name__ == '__main__':
    ...
    while len(s) % 4 != 0:
        s += ' '
    make_msg_array(s)
    # print("msg = ", convert_to_str())

    f = open("key.bin", 'rb')
    key = []
    for ln in f:
        x = int(ln)  # взять число
        key = key + [x]
    f.close()
    mars = Mars(key)
    print("Choose type of last block addition: ")
    print("0. ANSI X.923")
    print("1. ISO 10126")
    print("2. PKCS7")
    print("3. ISO/IEC 7816-4")
    last_type = int(input())

    blocks_encrypt = mars.encrypt(last_type)
    print("encrypt array: ")
    for x in blocks_encrypt:
        print(x.sub)
    blocks_decrypt = mars.decrypt(blocks_encrypt)
    # print(blocks_decrypt)
    str1 = convert_to_str(blocks_decrypt)
    print("decrypt text (last block is damaged) = ", str1)
    arr = without_last_block(blocks_decrypt, last_type)
    # print(arr)
    str2 = convert_to_str(arr)
    print("decrypt text (last block is like in start msg) = ", str2)
